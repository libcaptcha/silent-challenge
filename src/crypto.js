...
function hmacSha256(key, message) {
    ...
    const inner = new Uint8Array(HMAC_BLOCK + messageBytes.length);
    inner.set(innerKey);
...
function balloon(input, spaceCost, timeCost, delta = DELTA) {
    ...
    for (let t = 0; t < timeCost; t++) {
        for (let i = 0; i < spaceCost; i++) {
            ...
            for (let j = 0; j < delta; j++) {
                ...
                const other = (idx[0] >>> 0) % spaceCost;
                scratch.set(bBlock(buffer, i), 4);
                scratch.set(bBlock(buffer, other), 36);
                state = hashWithCounter(counter++, scratch, 68);
                digestToBytes(state, buffer, i * 32);
            }
        }
...
function balloonHex(input, spaceCost, timeCost, delta = DELTA) {
