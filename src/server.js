...
import { createChallengeBundle } from './vm.js';
...
export function silentMiddleware(options = {}) {
    ...
    function handleVmbc(request, response) {
        ...
        try {
            const bundle = createChallengeBundle(vmbcPath, token);
            response.set('Content-Type', 'application/octet-stream').send(bundle);
        } catch {
            response.status(500).json({ error: 'Bundle failed' });
        }
    }
    ...
    function handleVerify(request, response) {
        ...
        const result = manager.verify(challengeId, payload, headers);
    ...
    function requireToken(request, response, next) {
        ...
        const payload = manager.validateToken(header.slice(7));
        if (!payload) {
    ...
    function mountRoutes(router) {
        ...
        router.get('/challenge/:challengeId/vmbc', handleVmbc);
        router.post('/challenge/:challengeId/verify', handleVerify);
        return router;
    }
