...
function fractionNonInteger(arr) {
    if (!arr.length) return 0;
    const count = arr.filter((v) => !Number.isInteger(v)).length;
...
function createCollector(options = {}) {
    ...
    function onMouseMove(e) {
        ...
        if (data.m.length >= limits.mouse) data.m.shift();
        data.m.push([e.clientX + e.movementX * 0.01, e.clientY + e.movementY * 0.01, rt()]);
        pushEvent(0);
    }
    ...
    function onClick(e) {
        ...
        if (rect && rect.width > 0 && rect.height > 0) {
            ...
            data.c.push([e.clientX - cx, e.clientY - cy, dwell, rect.width, rect.height, now]);
        } else {
            data.c.push([0, 0, dwell, 0, 0, now]);
        }
        downTime = 0;

        for (const [element, index] of boundElements) {
    ...
    function onKeyUp(e) {
        ...
        if (downT && data.k.length < limits.keys) {
            ...
            const flight = lastKeyUp ? downT - lastKeyUp : 0;
    ...
    function onScroll() {
        ...
        const now = rt();
        pushEvent(6);
        if (data.s.length >= limits.scroll) data.s.shift();
        const prev = data.s.length ? data.s[data.s.length - 1][0] : 0;
        data.s.push([window.scrollY, window.scrollY - prev, now]);
    }
    ...
    function onDeviceMotion(e) {
        ...
        if (a && data.ac.length < limits.sensors) {
            data.ac.push([a.x || 0, a.y || 0, a.z || 0, rt()]);
        }
        const r = e.rotationRate;
        if (r && data.gy.length < limits.sensors) {
            data.gy.push([r.alpha || 0, r.beta || 0, r.gamma || 0, rt()]);
        }
    }

    function onDeviceOrientation(e) {
        if (data.or.length < limits.sensors) {
            data.or.push([e.alpha || 0, e.beta || 0, e.gamma || 0, rt()]);
        }
    }
    ...
    function attach(doc, win) {
        ...
        d.addEventListener('mousemove', onMouseMove, passive);
        d.addEventListener('mousedown', onMouseDown, passive);
        d.addEventListener('mouseup', onMouseUp, passive);
        d.addEventListener('click', onClick, passive);
        d.addEventListener('keydown', onKeyDown, passive);
        d.addEventListener('keyup', onKeyUp, passive);
        w.addEventListener('scroll', onScroll, passive);

        d.addEventListener('touchstart', (e) => onTouch(e, 7), passive);
        d.addEventListener('touchmove', (e) => onTouch(e, 8), passive);
        d.addEventListener('touchend', (e) => onTouch(e, 9), passive);

        data.meta.hasTouchScreen = 'ontouchstart' in w || navigator.maxTouchPoints > 0;
        data.meta.hasMotionSensors = false;

        if (w.DeviceMotionEvent) {
            try {
                if (typeof w.DeviceMotionEvent.requestPermission === 'function') {
                    data.meta.sensorPermissionRequired = true;
                }
            } catch {}
            w.addEventListener('devicemotion', onDeviceMotion, passive);
        }

        if (w.DeviceOrientationEvent) {
            w.addEventListener('deviceorientation', onDeviceOrientation, passive);
        }
    }
    ...
    function detach(doc, win) {
        ...
        w.removeEventListener('devicemotion', onDeviceMotion);
        w.removeEventListener('deviceorientation', onDeviceOrientation);
    }

    function isReady() {
        if (rt() < minTime) return false;
        return (
            data.m.length >= 10 || data.tc.length >= 5 || data.k.length >= 3 || data.s.length >= 3
        );
    }
...
function analyzeMouse(data) {
    ...
    if (curvatures.length >= 10) {
        ...
        if (curvEntropy < 1.0) {
            penalty += 0.12;
            result.reasons.push(
                `Low curvature entropy: ` + `${curvEntropy.toFixed(2)} ` + `(straight-line)`
            );
        } else if (curvEntropy < 1.8) {
            penalty += 0.05;
            result.reasons.push(
                `Below-average curvature ` + `entropy: ` + `${curvEntropy.toFixed(2)}`
            );
        }
    }
    ...
    if (points.length >= 10) {
        ...
        if (tremorRMS < 0.05) {
            penalty += 0.1;
            result.reasons.push(
                `No micro-tremor: RMS=` + `${tremorRMS.toFixed(3)}px ` + `(too smooth)`
            );
    ...
    if (velocities.length >= 10) {
        ...
        if (velCV < 0.15) {
            penalty += 0.12;
            result.reasons.push(`Constant velocity: ` + `CV=${velCV.toFixed(3)}`);
        } else if (velCV < 0.3) {
            penalty += 0.05;
    ...
    if (velocities.length >= 8) {
        ...
        if (jerks.length >= 5) {
            ...
            if (jerkCV < 0.2 && mean(jerks.map(Math.abs)) > 0) {
                penalty += 0.06;
                result.reasons.push(
                    `Low jerk variance: ` + `CV=${jerkCV.toFixed(3)} ` + `(too smooth)`
                );
            }
        }
    }
    ...
    if (points.length >= 20) {
        ...
        for (let i = 0; i <= points.length - windowSize; i += 5) {
            const seg = points.slice(i, i + windowSize);
            ...
            for (let j = 1; j < seg.length; j++) {
                pathLength += dist(seg[j - 1].x, seg[j - 1].y, seg[j].x, seg[j].y);
            }
            straightnessValues.push(pathLength / directDist);
        }

        if (straightnessValues.length >= 3) {
            const avg = mean(straightnessValues);
            if (avg < 1.005) {
                penalty += 0.1;
                result.reasons.push(`Perfectly straight paths: ` + `idx=${avg.toFixed(4)}`);
            } else if (avg < 1.015) {
                penalty += 0.04;
                result.reasons.push(`Very straight paths: ` + `idx=${avg.toFixed(4)}`);
            }
        }
    }
    ...
    for (let i = 1; i < points.length - 1; i++) {
        ...
        if (dx1 !== 0 || dy1 !== 0 || dx2 !== 0 || dy2 !== 0) {
            angles.push(Math.atan2(dx1 * dy2 - dy1 * dx2, dx1 * dx2 + dy1 * dy2));
        }
    }
    ...
    if (angles.length >= 15) {
        ...
        if (angleEntropy < 1.2) {
            penalty += 0.08;
            result.reasons.push(
                `Low direction entropy: ` + `${angleEntropy.toFixed(2)} ` + `(mechanical)`
            );
        }
    }
    ...
    if (intervals.length >= 10) {
        ...
        if (rounded.length >= 10) {
            ...
            if (modeRatio > 0.7 && mean(intervals) > 80) {
                penalty += 0.1;
                result.reasons.push(
    ...
    if (intervals.length >= 20) {
        const pauses = intervals.filter((i) => i > 150).length;
        const totalDur = points[points.length - 1].t - points[0].t;
        if (totalDur > 2000 && pauses < 2) {
            penalty += 0.06;
            result.reasons.push(
                `Continuous mouse movement: ` +
                    `${pauses} pauses in ` +
                    `${(totalDur / 1000).toFixed(1)}s`
            );
        }
    }
    ...
    if (velocities.length >= 20) {
        ...
        const minimaRate = dur > 0 ? minima / dur : 0;
    ...
    if (points.length >= 10) {
        ...
        const nonInt = fractionNonInteger(timestamps);
        if (nonInt > 0.5) {
            penalty += 0.1;
            result.reasons.push(
                `Non-integer mouse ` + `timestamps: ` + `${(nonInt * 100).toFixed(0)}% fractional`
            );
        }
    }

    const precisions = points.map((p) => Math.max(countDecimals(p.x), countDecimals(p.y)));
    const avgPrecision = mean(precisions);
    if (avgPrecision > 6) {
        penalty += 0.15;
        result.reasons.push(
            `Synthetic coordinate ` + `precision: avg=` + `${avgPrecision.toFixed(1)} ` + `decimals`
        );
    } else if (avgPrecision > 3) {
        penalty += 0.08;
        result.reasons.push(
            `Unusual coordinate ` + `precision: avg=` + `${avgPrecision.toFixed(1)} ` + `decimals`
        );
    }
    ...
    if (points.length >= 30) {
        ...
        let maxCorr = 0;
        const maxLag = Math.min(Math.floor(points.length / 2), 50);
        for (let lag = 5; lag <= maxLag; lag++) {
            const cx = autocorrelation(xVals, lag);
            const cy = autocorrelation(yVals, lag);
            maxCorr = Math.max(maxCorr, Math.abs(cx), Math.abs(cy));
        }
        if (maxCorr > 0.7) {
            penalty += 0.1;
            result.reasons.push(`Periodic movement detected: ` + `corr=${maxCorr.toFixed(3)}`);
        }
    }
...
function analyzeClicks(data) {
    ...
    for (const click of c) {
        ...
        if (tw > 0 && th > 0) {
            offsets.push(Math.sqrt((ox / (tw / 2)) ** 2 + (oy / (th / 2)) ** 2));
        }
    ...
    if (offsets.length >= 3) {
        ...
        if (centerRatio > 0.7) {
            penalty += 0.12;
            result.reasons.push(
                `${(centerRatio * 100).toFixed(0)}% ` + `center clicks ` + `(pixel-perfect)`
            );
        } else if (centerRatio > 0.5) {
            penalty += 0.06;
            result.reasons.push(
                `${(centerRatio * 100).toFixed(0)}% ` + `center clicks (suspicious)`
            );
        }
        ...
        if (offsetStd < 0.02 && offsets.length >= 5) {
            penalty += 0.08;
            result.reasons.push(
                `Click offset variance ` + `too low: std=` + `${offsetStd.toFixed(4)}`
            );
        }
    }
    ...
    if (dwells.length >= 3) {
        ...
        if (avgDwell < 10) {
            ...
            penalty += 0.06;
            result.reasons.push(
                `Perfectly uniform click ` + `duration: ` + `CV=${dwellCV.toFixed(3)}`
            );
        }
    ...
    if (dwells.length >= 3) {
        const nonInt = fractionNonInteger(dwells);
        if (nonInt > 0.5) {
            penalty += 0.08;
            result.reasons.push(
                `Non-integer click dwell ` + `times: ` + `${(nonInt * 100).toFixed(0)}% fractional`
            );
        }
    }
...
function analyzePreClick(data) {
    ...
    if (totalAnalyzed >= 2 && noDecelCount === totalAnalyzed) {
        penalty += 0.08;
        result.reasons.push(
            `No pre-click deceleration ` + `in ${noDecelCount}/` + `${totalAnalyzed} clicks`
        );
...
function analyzeKeystrokes(data) {
    ...
    if (dwells.length >= 3) {
        ...
        if (avgDwell < 5) {
            penalty += 0.1;
            result.reasons.push(
                `Key dwell impossibly ` + `short: avg=` + `${avgDwell.toFixed(1)}ms`
            );
    ...
    if (timings.length >= 10) {
        ...
        if (rhythmEntropy < 1.5) {
            penalty += 0.06;
            result.reasons.push(
                `Low rhythm entropy: ` + `${rhythmEntropy.toFixed(2)} ` + `(mechanical)`
            );
        }
    }

    const allTimings = [...dwells, ...flights];
    if (allTimings.length >= 6) {
        const nonInt = fractionNonInteger(allTimings);
        if (nonInt > 0.5) {
            penalty += 0.1;
            result.reasons.push(
                `Non-integer key timing: ` + `${(nonInt * 100).toFixed(0)}% fractional`
            );
        }
    }
...
function analyzeScroll(data) {
    ...
    if (reversals === 0 && deltas.length >= 10) {
        penalty += 0.04;
    ...
    if (times.length >= 3) {
        const nonInt = fractionNonInteger(times);
        if (nonInt > 0.5) {
            penalty += 0.05;
            result.reasons.push(
                `Non-integer scroll ` + `timestamps: ` + `${(nonInt * 100).toFixed(0)}% fractional`
            );
        }
    }
    ...
    if (s.length >= 3) {
        ...
        const posNonInt = fractionNonInteger(positions);
        if (posNonInt > 0.5) {
            penalty += 0.04;
            result.reasons.push(
                `Non-integer scroll ` +
                    `positions: ` +
                    `${(posNonInt * 100).toFixed(0)}% fractional`
            );
        }
    }
...
function analyzeTouch(data) {
    ...
    for (const gesture of gestures) {
        ...
        if (wobbleRMS < 0.3 && gesture.length >= 8) {
            penalty += 0.04;
            result.reasons.push(
                `Geometrically perfect ` + `swipe: wobble=` + `${wobbleRMS.toFixed(2)}px`
            );
        }
...
function analyzeSensors(data) {
    ...
    if (or && or.length >= 5) {
        ...
        const duration = (last[3] - first[3]) / 1000;

        if (totalDrift < 0.01 && duration > 3) {
            penalty += 0.04;
            result.reasons.push('Zero orientation drift ' + '(perfectly static)');
        }
    }
...
function analyzeEventOrder(data) {
    ...
    for (const [type] of ev) {
        if (type === 4) pendingKeys.add(type);
        if (type === 5) {
    ...
    for (let i = 1; i < ev.length; i++) {
        if (ev[i - 1][0] === 1 && ev[i][0] === 2 && ev[i][1] === ev[i - 1][1]) {
            penalty += 0.03;
            result.reasons.push('Zero-time mousedown/' + 'mouseup (dispatched)');
            break;
        }
    }
    ...
    for (const [type] of ev) {
        ...
        if (type === 8 && !inTouch) touchMovesWithoutStart++;
        if (type === 9) inTouch = false;
    }
    if (touchMovesWithoutStart > 0) {
        penalty += 0.02;
        result.reasons.push(`${touchMovesWithoutStart} ` + `touchmove without touchstart`);
    }
...
function analyzeEngagement(data) {
    ...
    if (dur > 0 && dur < 500) {
        ...
        if (totalEvents > 50) {
            penalty += 0.03;
            result.reasons.push(`${totalEvents} events in ` + `${dur}ms (burst)`);
        }
    }
...
function analyzeSyntheticEvents(data) {
    ...
    const fastClicks =
        c && c.length >= 2 && c.filter((click) => click[2] < 5).length > c.length * 0.5;
    const fastKeys = k && k.length >= 3 && k.filter((key) => key[0] < 5).length > k.length * 0.5;

    if (fastClicks && fastKeys) {
        penalty += 0.1;
        result.reasons.push(
            'Clicks and keystrokes both ' + 'impossibly fast ' + '(automated dispatch)'
        );
    } else if (fastClicks) {
        penalty += 0.04;
        result.reasons.push('Automated click dispatch ' + 'pattern');
    } else if (fastKeys) {
        penalty += 0.04;
        result.reasons.push('Automated keystroke dispatch ' + 'pattern');
    }
    ...
    if (ev && ev.length >= 10) {
        ...
        if (zeroTimeClicks >= 2) {
            penalty += 0.05;
            result.reasons.push(`${zeroTimeClicks} zero-time ` + `mousedown/mouseup pairs`);
        }
    }
...
function analyze(data) {
    const categories = {
        ...
        synthetic: analyzeSyntheticEvents(data),
    };
