...
const keyRing = createKeyRing({
    ...
    compileCommand:
        'node scripts/compile.cjs web/attestation.js'
        + ' --out web/attestation.vmbc',
    onRotate: (manifest) => {
        console.log(
            `[vm] rotated keys: ${manifest.buildId
                || 'generated'}`,
        );
    },
});
...
const silent = silentMiddleware({
    ...
    vmbcPath: keyRing.built
        ? join(vmDir, 'web', 'attestation.vmbc')
        : null,
    pow: {
        difficulty: parseInt(
            process.env.POW_DIFFICULTY || '10', 10,
        ),
        spaceCost: parseInt(
            process.env.POW_SPACE_COST || '512', 10,
        ),
    },
    thresholds: {
        combined: parseFloat(
...
app.use(
    '/src',
    express.static(join(root, 'src')),
);
app.use(
    '/vm/web',
    express.static(join(root, 'vm', 'web')),
);
